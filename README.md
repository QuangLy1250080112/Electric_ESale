...
* 🖥️ **Frontend React** chạy tại: [http://localhost:5173](http://localhost:5173)
* ⚙️ **Backend API (Swagger Docs)** chạy tại: [http://localhost:8000/docs](http://localhost:8000/docs)

---

## 🛠️ Cấu Hình Nâng Cao Cho Lập Trình Viên (Local PostgreSQL)

Nếu bạn muốn cấu hình chạy dự án này trên môi trường phát triển local kết nối với cơ sở dữ liệu PostgreSQL (hoặc pgadmin4) riêng:
...
*(File `backend/.env` này đã được liệt kê trong `.gitignore` để không đè lên cấu hình cơ sở dữ liệu SQLite mặc định của giáo viên khi đẩy lên GitHub).*