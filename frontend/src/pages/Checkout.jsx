...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((leafletModule) => {
      ...
      const shopLat = state.shopLocation.lat,
        shopLng = state.shopLocation.lng;
      ...
      L.marker([shopLat, shopLng], { icon: redIcon })
        .addTo(map)
        .bindPopup("Cửa hàng ESale");
      ...
      L.polyline(
        ...
        { color: "#2563eb", weight: 3, dashArray: "10,10" },
      ).addTo(map);
      ...
      const bike = L.marker([shopLat, shopLng], { icon: greenIcon })
        .addTo(map)
        .bindPopup("🏍️ Shipper");
      bikeMarkerRef.current = bike;

      const totalTime =
        state.distance * (state.deliverySecondsPerKm || 5) * 1000;
      const fps = 30;
  ...
  const steps = [
    {
      ...
      label: "Đang tiếp nhận đơn",
      desc: "Cửa hàng đang xử lý đơn hàng của bạn...",
    },
    {
      icon: <CheckCircle size={28} />,
      label: "Đã xác nhận đơn hàng",
      desc: "Đơn hàng đã được xác nhận thành công!",
    },
    {
      icon: <Truck size={28} />,
      label: "Đang giao hàng",
      desc: `Shipper đang trên đường đến (${state.distance?.toFixed(1)} km)`,
    },
    {
      icon: <CheckCircle size={28} />,
      label: "Giao hàng thành công!",
      desc: "Đơn hàng đã được giao tới bạn.",
    },
  ];
          ...
          <div key={i} style={{ textAlign: "center", zIndex: 1, flex: 1 }}>
            <div
              style={{
                ...
                boxShadow: step === i ? "0 0 20px rgba(37,99,235,0.4)" : "none",
              }}
            >
              {s.icon}
            </div>
      ...
      >
        ...
        {step === 3 && (
          <div className="animate-fade-in">
            ...
            <p style={{ color: "#666", marginBottom: "1rem" }}>
              Cảm ơn bạn đã mua hàng. Bạn có thể đánh giá sản phẩm ngay bây giờ.
            </p>
            <button
